    ...
    class StudentCollection
    {
        ...
        public void AddDefaults(int n) {
            for (int i = 1; i <= n; i++) {
                ...
                StudentCountChanged(this, new StudentListHandlerEventArgs(CollectionName, "Added", obj));
            }
        }
        ...
        public void AddStudents(Student[] studarr) {
            foreach (Student stud in studarr)
            {
                ...
                StudentCountChanged(this, new StudentListHandlerEventArgs(CollectionName, "Added", stud));
            }
        }


        public bool Remove(int j) {
            if (j >= 0 && j<=StudentsList.Count-1)
            {
                StudentCountChanged(this, new StudentListHandlerEventArgs(CollectionName, "Removed", StudentsList.ElementAt(j)));
                StudentsList.RemoveAt(j);
                return true;
            }
            return false;
        }
        ...
        public Student this[int i] {
            get { return StudentsList[i]; }
            set {
                StudentsList[i] = value;
                StudentReferenceChanged(this, new StudentListHandlerEventArgs(CollectionName, "Changed", StudentsList[i]));
            }
        }
